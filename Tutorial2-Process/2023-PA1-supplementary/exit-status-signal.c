...
int main() {
  ...
  if (pid < 0) {
    ...
    // sleep 1 second to avoid killing too quickly
    sleep(1);
    // send SIGTERM to child process to kill it
    kill(pid, SIGTERM);
    ...
    if (!ret) {
      ...
      if (WIFSIGNALED(status)) { // check if is killed
        printf("Child process %d has been killed by signal %d\n", (int) info.si_pid, WTERMSIG(status));
      }
    } else {
      perror("waitid");
    }
